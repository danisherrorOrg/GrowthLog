...
const resolveRank = (group, mapping, mode) => {
  ...
  if (mode === 'composite') return data.rank;
  if (mode === 'volume') {
    if (data.volume >= 8000) return 'Peak';
    if (data.volume >= 4000) return 'Heavy';
    if (data.volume >= 2000) return 'Moderate';
    if (data.volume >= 500) return 'Light';
    return 'Untrained';
  }
  if (mode === 'recency') {
    if (data.days_ago === '—' || data.days_ago > 10) return 'Untrained';
    if (data.days_ago <= 2) return 'Peak';
    if (data.days_ago <= 5) return 'Heavy';
    if (data.days_ago <= 7) return 'Moderate';
    return 'Light';
  }
  return 'Untrained';
};
...
const getProgress = (group, mapping, mode) => {
  const data = mapping[group];
  if (!data) return 0;
  if (mode === 'composite') return data.score || 0;
  if (mode === 'volume') return Math.min(100, (data.volume / 10000) * 100);
  if (mode === 'recency') return data.days_ago === '—' ? 0 : Math.max(0, 100 - (data.days_ago / 10 * 100));
  return 0;
};
...
/* ─────────────────────────────────────────────
   Tooltip
───────────────────────────────────────────── */
function MuscleTooltip({ tooltip, viewMode }) {
  if (!tooltip) return null;
  const { label, group, x, y, volume, maxWeight, sets, score, days_ago } = tooltip;

  // Create a synthetic data mapping for the hovered tooltip since backend returns full list
  const synthMapping = { [group]: tooltip };
  const displayRank = resolveRank(group, synthMapping, viewMode);

  const meta = RANK_META[displayRank] || RANK_META.Untrained;

  return (
    <div style={{
      position: 'fixed', left: x + 18, top: y - 80,
      ...
      <div style={{
        ...
        borderRadius: 16, padding: '14px 18px', minWidth: 210,
        boxShadow: `0 20px 50px rgba(0,0,0,0.5), 0 0 28px ${meta.glow}`,
      }}>
        {/* Rank badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
          <div style={{
            background: meta.badge, borderRadius: 8,
        ...
        <div style={{ fontSize: 11, color: meta.color, fontWeight: 700, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
          {group} Group
        </div>
        {/* Stats grid */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[
            ...
            { label: 'RECENCY', value: days_ago === '—' ? 0 : days_ago, unit: 'days ago' },
            { label: 'VOLUME', value: (volume || 0).toLocaleString(), unit: 'kg' },
          ].map(s => (
            <div key={s.label} style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 10, padding: '8px 11px' }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.38)', letterSpacing: 1 }}>{s.label}</div>
              <div style={{ fontWeight: 900, fontSize: 15, color: '#fff' }}>
                {s.value} <span style={{ fontWeight: 400, fontSize: 10, color: 'rgba(255,255,255,0.38)' }}>{s.unit}</span>
              </div>
            </div>
          ))}
        </div>
...
/* ─────────────────────────────────────────────
   Body SVG — light skin, vivid muscle colours
───────────────────────────────────────────── */
function BodySVG({ isBack, mapping, onHover, onLeave, viewMode }) {
  const muscles = isBack ? BACK_MUSCLES : FRONT_MUSCLES;

  return (
    <svg width="210" height="415" viewBox="0 0 200 410"
      ...
      {muscles.map((m) => {
        // Non-interactive structural piece
        if (!m.label) {
          return (
        ...
        return (
          <path
            ...
            filter={!isUnt ? `url(#glow-${displayRank})` : undefined}
            ...
            onMouseMove={(e) => onHover(p => p ? { ...p, x: e.clientX, y: e.clientY } : p)}
            onMouseLeave={(e) => {
...
export default function BodyMapVisualizer() {
  ...
  const handleLeave = () => { setTooltip(null); setActiveGroup(null); };

  /* ── Loading skeleton ── */
  if (loading) return (
    <div style={{ padding: '30px 0' }}>
      <div style={{ height: 480, borderRadius: 24, background: 'rgba(13,13,13,0.04)', animation: 'bmPulse 1.6s infinite' }} />
    </div>
  );
    ...
    <div style={{ padding: '0 4px' }}>
      ...
      <style>{`
        ...
        .legend-tooltip::after {
          ...
          border-left: 4px solid transparent; border-right: 4px solid transparent; border-top: 4px solid rgba(15,23,42,0.95);
        }
      ...
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(13,13,13,0.5)', maxWidth: 420, margin: '0 auto', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 52, flexWrap: 'wrap', marginBottom: 50 }}>
        {[{ label: 'FRONT VIEW', isBack: false }, { label: 'BACK VIEW', isBack: true }].map(({ label, isBack }) => (
          <div key={label} style={{ textAlign: 'center' }}>
            ...
            <div style={{
              ...
              <BodySVG isBack={isBack} mapping={mapping} onHover={handleHover} onLeave={handleLeave} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 14 }}>
        {MUSCLE_GROUPS.map((group) => {
          const data = mapping[group] || {};
          const displayRank = resolveRank(group, mapping, 'composite'); // Force side panel to show composite text logically, independent of map toggle
          const meta = RANK_META[displayRank] || RANK_META.Untrained;
          ...
          const prog = getProgress(group, mapping, 'composite');
          const isPeak = displayRank === 'Peak';
          const isUnt = displayRank === 'Untrained';
            ...
            <div key={group} className="bm-card" style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: isPeak
                    ? 'linear-gradient(to right,#e11d48,#f97316)'
                    : meta.color,
                  borderRadius: 99,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {[
                  ...
                  { label: 'RECENCY', val: String(d_ago), unit: 'days' },
                ].map(s => (
                  <div key={s.label} style={{
                    ...
                    <div style={{ fontSize: 9, fontWeight: 700, color: 'rgba(13,13,13,0.38)', letterSpacing: 1 }}>{s.label}</div>
                    <div style={{ fontWeight: 900, fontSize: 15, color: 'var(--ink)' }}>
                      {s.val} <span style={{ fontWeight: 400, fontSize: 9, color: 'rgba(13,13,13,0.38)' }}>{s.unit}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {!isPeak && !isUnt && (
                <div style={{ marginTop: 10, fontSize: 10, color: 'rgba(13,13,13,0.4)', fontWeight: 700 }}>
                  Score: {score} → {RANKS[RANKS.indexOf(displayRank) + 1]}
                </div>
              )}
            </div>
          );
      ...
      {/* Floating tooltip */}
      <MuscleTooltip tooltip={tooltip} viewMode={viewMode} />
    </div>
  );
}
