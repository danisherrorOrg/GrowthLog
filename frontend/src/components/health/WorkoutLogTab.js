...
/* ── Anatomically-detailed muscle paths (matches BodyMapVisualizer) ── */
...
/* ── Mini body selector — anatomical quality matching BodyMapVisualizer ── */
function MiniBodySelector({ selected, onChange }) {
  ...
  const renderFigure = (muscles, label) => (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontSize: 9, fontWeight: 900, letterSpacing: 2.5, color: 'rgba(13,13,13,0.28)', marginBottom: 8 }}>{label}</div>
      <svg width="130" height="267" viewBox="0 0 200 410" style={{ overflow: 'visible', filter: 'drop-shadow(0 4px 14px rgba(0,0,0,0.12))' }}>
        <defs>
          {Object.entries(GROUP_COLORS).map(([grp, m]) => (
            <filter key={grp} id={`mgf-${grp}`} x="-30%" y="-30%" width="160%" height="160%">
        ...
        {muscles.map(m => {
          if (m.s) return <path key={m.id} d={m.path} fill={m.f} stroke={m.sk} strokeWidth={m.sw} />;
          const mc = GROUP_COLORS[m.group]?.fill || '#94a3b8';
          const on = m.group === selected;
          return (
            <path
              key={m.id} d={m.path}
              fill={mc} fillOpacity={on ? 0.82 : 0.15}
              stroke={mc} strokeWidth={on ? 2.2 : 1.2} strokeOpacity={on ? 1 : 0.3}
              filter={on ? `url(#mgf-${m.group})` : undefined}
              style={{ cursor: 'pointer', transition: 'fill-opacity 0.16s' }}
              onClick={() => onChange(m.group)}
              onMouseEnter={e => { if (!on) { e.currentTarget.style.fillOpacity='0.45'; e.currentTarget.style.strokeOpacity='0.8'; } }}
              onMouseLeave={e => { if (!on) { e.currentTarget.style.fillOpacity='0.15'; e.currentTarget.style.strokeOpacity='0.3'; } }}
            />
          );
        })}
      </svg>
  ...
  return (
    <div style={{ background: light, border: `1.5px solid ${color}35`, borderRadius: 14, padding: '12px 14px 10px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 10, color: 'rgba(13,13,13,0.35)', fontWeight: 600 }}>tap muscle to change</span>
      </div>
      {/* Front + Back figures */}
      ...
      {/* Cardio (not on body) */}
      <div style={{ textAlign: 'center' }}>
        <button type="button" onClick={() => onChange('Cardio')} style={{
...
export default function WorkoutLogTab() {
  ...
  // Custom exercises per user (keyed by muscle group)
  ...
  const loadCustomExercises = async () => {
    try {
      ...
      // silent fail — custom exercises are a non-critical feature
    }
  };

  const saveCustomExercise = async (muscleGroup, exerciseName) => {
    try {
  ...
  const isAlreadySaved = (muscleGroup, exerciseName) => {
    const defaults = EXERCISES_BY_GROUP[muscleGroup] || [];
    const customs  = (customExercises[muscleGroup] || []).map(c => c.exercise_name);
    return defaults.includes(exerciseName) || customs.includes(exerciseName);
  };
  ...
  const openAdd = () => {
    ...
    setExercises([{ exercise_name: '', muscle_group: 'Chest', sets: [{ reps: 10, weight: 0, distance_km: 0, duration_min: 30 }], notes: '' }]);
    setShowModal(true);
  };
  ...
  const openEdit = (w) => {
    ...
    // Normalise sets so cardio fields always exist
    ...
    setExercises(normalised.length > 0 ? normalised : [{ exercise_name: '', muscle_group: 'Chest', sets: [{ reps: 10, weight: 0, distance_km: 0, duration_min: 30 }], notes: '' }]);
    setViewingWorkout(null);
    setShowModal(true);
  };

  const handleAddExercise = () => {
    setExercises([...exercises, { exercise_name: '', muscle_group: 'Chest', sets: [{ reps: 10, weight: 0, distance_km: 0, duration_min: 30 }], notes: '' }]);
  };
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setViewingWorkout(null)}>
          <div className="modal" style={{ maxWidth: 650, maxHeight: '90vh', overflowY: 'auto', padding: 32 }}>
                      ...
                      <div key={sIdx} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(13,13,13,0.02)', borderRadius: 10 }}>
                        ...
                        {e.muscle_group === 'Cardio' ? (
                          <div style={{ fontWeight: 800 }}>
                            {s.distance_km > 0 ? <>{s.distance_km} <span style={{ fontWeight: 400, opacity: 0.5 }}>km</span> · </> : ''}
                            {s.duration_min} <span style={{ fontWeight: 400, opacity: 0.5 }}>min</span>
                          </div>
                        ) : (
                          <div style={{ fontWeight: 800 }}>
                            {s.reps} <span style={{ fontWeight: 400, opacity: 0.5 }}>reps</span>
                            {' · '}{s.weight > 0 ? <>{s.weight} <span style={{ fontWeight: 400, opacity: 0.5 }}>kg</span></> : 'Bodyweight'}
                          </div>
                        )}
                      </div>
                    ))}
            ...
            <div style={{ marginTop: 40, display: 'flex', gap: 12 }}>
              <button className="btn btn-outline" style={{ flex: 1 }} onClick={() => openEdit(viewingWorkout)}>✎ Edit Session</button>
          ...
          <div className="modal" style={{ maxWidth: 780, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginTop: 20, marginBottom: 20 }}>
              ...
              {exercises.map((e, exIdx) => {
                ...
                const suggestions = EXERCISES_BY_GROUP[e.muscle_group] || ALL_EXERCISES;
                ...
                return (
                  <div key={exIdx} style={{
                    ...
                    {/* ── Exercise name row ── */}
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '12px 14px 10px', borderBottom: `1px solid ${grpColor}15` }}>
                      <div style={{ flex: 1 }}>
                        <datalist id={listId}>
                          {/* Custom exercises appear first with a star prefix */}
                          {(customExercises[e.muscle_group] || []).map(c => (
                            <option key={`c-${c.id || c.exercise_name}`} value={c.exercise_name} />
                          ))}
                      ...
                      {/* Save to my list button — shows when exercise name is new */}
                    ...
                    {/* ── Body selector + sets side-by-side ── */}
                    <div style={{ display: 'flex', gap: 0 }}>
                      ...
                      <div style={{ flex: 1, padding: '14px 14px 12px' }}>
                        <div style={{ fontSize: 10, fontWeight: 900, color: 'rgba(13,13,13,0.35)', letterSpacing: 1.5, marginBottom: 10 }}>
                        ...
                        {e.sets.map((s, setIdx) => (
                          <div key={setIdx} style={{
                            ...
                            <span style={{ fontSize: 10, fontWeight: 800, color: grpColor, width: 42, flexShrink: 0 }}>
                              {isCardio ? `RND ${setIdx + 1}` : `SET ${setIdx + 1}`}
                            </span>
                            {isCardio ? (
                              <>
                                <input type="number" step="0.1" className="form-input" placeholder="km" value={s.distance_km} onChange={ev => updateSet(exIdx, setIdx, 'distance_km', ev.target.value)} style={{ width: 64, padding: '4px 8px', height: 30, fontSize: 13 }} />
                                <span style={{ fontSize: 11, color: 'rgba(13,13,13,0.4)', flexShrink: 0 }}>km</span>
                                <input type="number" className="form-input" placeholder="min" value={s.duration_min} onChange={ev => updateSet(exIdx, setIdx, 'duration_min', ev.target.value)} style={{ width: 64, padding: '4px 8px', height: 30, fontSize: 13 }} />
                                <span style={{ fontSize: 11, color: 'rgba(13,13,13,0.4)', flexShrink: 0 }}>min</span>
                              </>
                            ) : (
                              <>
                                <input type="number" className="form-input" placeholder="Reps" value={s.reps} onChange={ev => updateSet(exIdx, setIdx, 'reps', ev.target.value)} style={{ width: 60, padding: '4px 8px', height: 30, fontSize: 13 }} />
                                <span style={{ fontSize: 11, color: 'rgba(13,13,13,0.4)', flexShrink: 0 }}>×</span>
                                <input type="number" step="0.5" className="form-input" placeholder="kg" value={s.weight} onChange={ev => updateSet(exIdx, setIdx, 'weight', ev.target.value)} style={{ width: 64, padding: '4px 8px', height: 30, fontSize: 13 }} />
                                <span style={{ fontSize: 11, color: 'rgba(13,13,13,0.4)', flexShrink: 0 }}>kg</span>
                              </>
                            )}
                            {e.sets.length > 1 && (
                              <button className="btn btn-ghost btn-sm" onClick={() => handleRemoveSet(exIdx, setIdx)} style={{ padding: '2px 6px', opacity: 0.4, marginLeft: 'auto' }}>−</button>
                            )}
                          </div>
                        ))}
                        <button
                          onClick={() => handleAddSet(exIdx)}
                          style={{ background: 'none', border: 'none', color: grpColor, fontSize: 12, fontWeight: 800, cursor: 'pointer', padding: '4px 2px', marginTop: 2 }}
                        >
